...
```math
R \equiv \frac{A_c}{A_s} \approx \frac{N_c}{N_s},
```
...
```math
\pi = 4R \approx \frac{4 N_c}{N_s}.
```
The code uses $`N_s=10,000`$ points generated within a square centered on the origin. $`N_c`$ is the number
of points whos distance, $`r`$, from the origin is less than unity. This is calculated using the Pythagorean theorem:
```math
r = \sqrt{x^2 + y^2},
```
